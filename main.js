...
// สร้างระบบควบคุมเว็บทั้งหมดในชื่อ 'app'
const app = {
    // state คือ "ความจำ" ของเว็บ ใช้จำว่าตะกร้ามีของอะไรบ้าง ตอนนี้อยู่หน้าไหน
    state: {
        cart: JSON.parse(localStorage.getItem('cart')) || [], // ดึงของในตะกร้าจากความจำมือถือ ถ้าไม่มีให้ว่างไว้
        currentView: 'home', 
    ...
    // init คือ ฟังก์ชันแรกที่จะทำงานเมื่อเว็บโหลดเสร็จ
    init() {
        this.home.renderCategories(); // สร้างปุ่มหมวดหมู่
        this.home.filter(); // ค้นหาสินค้ามาแสดง
        this.cart.updateUI(); // อัปเดตตัวเลขในตะกร้า
        lucide.createIcons(); // เสกไอคอนให้ออกมา
        
        // รองรับการกดปุ่ม "ย้อนกลับ (Back)" ของมือถือ
    ...
    // navigate คือ ฟังก์ชันสำหรับสลับหน้าจอไปมา (หน้าแรก หน้าสินค้า หน้าเกี่ยวกับร้าน)
    navigate(view, id = null, pushState = true) {
        ...
        // ซ่อนแถบเมนูด้านบน ถ้าอยู่ในหน้ารายละเอียดสินค้า
        const nav = document.getElementById('main-nav'); 
        if(nav) nav.style.display = (view === 'product') ? 'none' : 'flex';
        
        // ถ้าเปิดหน้าสินค้า ให้โหลดข้อมูลสินค้านั้นๆ
        if (view === 'product' && id) this.product.render(id); 
        else if (view === 'home') this.home.filter();
        
        // บันทึกประวัติการเข้าชม (เพื่อให้กดปุ่ม Back ได้)
        if (pushState) {
            const currentUrl = new URL(window.location.href);
            if (view === 'home') currentUrl.searchParams.delete('product'); 
            else currentUrl.searchParams.set('product', id);
            window.history.pushState({ view, id }, '', currentUrl.toString());
        }
        lucide.createIcons();
    },

    // ระบบจัดการใน "หน้าแรก" (แสดงหมวดหมู่ ค้นหา และแสดงสินค้า)
    home: {
        // สร้างปุ่มหมวดหมู่ (เช่น ดอกไม้เดี่ยว, ช่อดอกไม้)
        renderCategories() {
        ...
        // เมื่อกดเลือกหมวดหมู่ ให้จำหมวดนั้นแล้วค้นหาใหม่
        setCategory(cat) { 
            app.state.home.category = cat; 
            this.renderCategories(); 
            this.filter(); 
        },
        
        // กรองสินค้าตาม "คำค้นหา" และ "หมวดหมู่" ที่เลือก
        filter() {
            const val = document.getElementById('search-input')?.value.toLowerCase() || ''; 
            app.state.home.search = val;
            
            const filtered = products.filter(p => 
                (app.state.home.category === 'ทั้งหมด' || p.category === app.state.home.category) && 
                (p.name.toLowerCase().includes(val) || (p.description && p.description.toLowerCase().includes(val)))
            );
            
            this.renderGrid(filtered); // ส่งของที่กรองแล้วไปวาดบนจอ
        ...
        // เอาข้อมูลสินค้ามาวาดเป็นการ์ดโชว์บนหน้าเว็บ
        renderGrid(list) {
                ...
                <div onclick="app.navigate('product', ${p.id})" class="group bg-white rounded-2xl p-2 shadow-sm border-2 border-border/60 cursor-pointer flex flex-col relative transition-all hover:-translate-y-2 hover:shadow-xl hover:border-primary/60">
                    ...
                    <div class="px-2 pb-2 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto flex justify-between items-end pt-2">
                            ...
                            <button onclick="event.stopPropagation(); app.cart.openQtyModal(${p.id})" class="w-8 h-8 bg-[#E85D9F] text-white rounded-full flex items-center justify-center hover:scale-110 transition-all">
    ...
    // ระบบจัดการใน "หน้ารายละเอียดสินค้า"
    product: {
        // วาดหน้ารายละเอียดเมื่อกดเข้ามาดูสินค้า
        render(id) {
            ...
            // เตรียมรูปภาพย่อยๆ (ถ้ามี)
            let subImagesHtml = '';
            if(p.subImages && p.subImages.length > 0) {
                subImagesHtml = `<div class="flex gap-2 mt-3 px-4 md:px-0">` + p.subImages.map(img => `
                    <div class="w-16 h-16 rounded-xl overflow-hidden border-2 border-transparent hover:border-primary cursor-pointer transition-colors" onclick="document.getElementById('main-img-${p.id}').src='${img}'">
                        <img src="${img}" class="w-full h-full object-cover" onerror="this.style.display='none'">
                    </div>`).join('') + `</div>`;
            }
            
           // ใส่ข้อมูล รูปภาพ ราคา และรายละเอียดต่างๆ ลงไป
           container.innerHTML = `
                <div class="fixed top-0 left-0 right-0 z-40 px-4 py-3 flex items-center w-full">
                    <button onclick="app.navigate('home')" class="w-11 h-11 bg-white/95 backdrop-blur-md rounded-full flex items-center justify-center shadow-md border border-border/50"><i data-lucide="chevron-left" class="w-6 h-6"></i></button>
                </div>
                ...
                <div class="fixed bottom-0 left-0 right-0 z-50 bg-white px-4 py-3 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] border-t border-border/50">
                    <div class="max-w-5xl mx-auto flex gap-3 justify-end items-center">
                        <button onclick="const opt = document.getElementById('item-option') ? document.getElementById('item-option').value : ''; app.cart.openQtyModal(${p.id}, opt);" class="w-full h-[54px] bg-[#E85D9F] hover:bg-pink-600 text-white rounded-full font-medium flex items-center justify-center gap-2 shadow-md transition-all text-[16px]">
            ...
            if(floatBtn) this.initDrag(floatBtn); // ทำให้ปุ่มลอยลากได้
        },

        // สลับแท็บระหว่าง "เรื่องราว" กับ "สเปกขนาด" ในหน้ารายละเอียด
        setTab(tabId, obj = null) {
            ...
            if(content) {
                content.innerHTML = tabId === 'story' 
                    ? `<div class="animate-fade-in space-y-4"><p class="text-[14px] md:text-[16px] font-sans leading-relaxed">${p.description}</p>${p.story ? `<div class="bg-background/80 p-4 md:p-5 rounded-xl border border-primary/10"><h4 class="font-display font-medium text-primary mb-2 text-sm md:text-base flex items-center gap-2"><i data-lucide="sparkles" class="w-4 h-4"></i> เรื่องราว</h4><p class="text-[13px] md:text-[15px] text-muted-foreground leading-relaxed font-sans">${p.story}</p></div>`:''}</div>`
                    : `<div class="animate-fade-in space-y-5 font-sans">${p.dimensions ? `<div><div class="text-[12px] md:text-[14px] text-muted-foreground mb-1.5 flex items-center gap-2"><i data-lucide="ruler" class="w-4 h-4"></i> ขนาด</div><div class="text-[14px] md:text-[16px] pl-6">${p.dimensions}</div></div>`:''}${p.materials ? `<div><div class="text-[12px] md:text-[14px] text-muted-foreground mb-2 flex items-center gap-2"><i data-lucide="layers" class="w-4 h-4"></i> วัสดุ</div><ul class="text-[14px] md:text-[16px] space-y-2 pl-6 list-disc marker:text-primary/50">${p.materials.map(m=>`<li>${m}</li>`).join('')}</ul></div>`:''}</div>`;
            }
            lucide.createIcons();
        },

        // ระบบจับพิกัดเพื่อให้ลากปุ่มตะกร้าสีชมพูไปมาบนหน้าจอได้
        initDrag(elmnt) {
            ...
            elmnt.onclick = function(e) { if (!isDragging) { app.cart.toggle(); } }; // ถ้าไม่ได้ลาก แต่กดเฉยๆ ให้เปิดตะกร้า
    ...
    // ระบบจัดการ "ตะกร้าสินค้า" ทั้งหมด
    cart: {
        // เปิด-ปิด หน้าต่างสไลด์ตะกร้าสินค้า
        toggle() {
            ...
            if(drawer.classList.contains('hidden')) { 
                ...
                content.classList.add('translate-y-full', 'md:translate-x-full'); overlay.classList.replace('opacity-100', 'opacity-0'); 
                setTimeout(() => drawer.classList.add('hidden'), 300); 
            }
        },
        
        // จัดการหน้าต่างระบุจำนวนสินค้า (Modal) ก่อนเอาลงตะกร้า
        ...
        // การเอาสินค้าลงตะกร้า และบันทึกข้อมูล
        add(id, qty = 1, option = '') {
            const p = products.find(x => x.id === id); const exist = app.state.cart.find(x => x.id === id && x.option === option);
            if (exist) { exist.qty += qty; exist.selected = true; } // ถ้ามีของอยู่แล้ว ให้เพิ่มแค่จำนวน
            else { app.state.cart.push({ ...p, qty, selected: true, option: option }); } // ถ้ายังไม่มี ให้ใส่ของชิ้นใหม่ลงไป
        ...
        save() { localStorage.setItem('cart', JSON.stringify(app.state.cart)); }, // จำของลงโทรศัพท์ รีเฟรชก็ไม่หาย
        
        // ================= ฟังก์ชันหัวใจหลัก: การกดสั่งซื้อ คัดลอกข้อความ ไปยังแอปต่างๆ =================
        checkout(platform) {
...
แจ้งชื่อ ที่อยู่ และเบอร์โทรสำหรับจัดส่งได้เลยนะคะ 🤍`;
            ...
            try {
                if (navigator.clipboard && window.isSecureContext) {
                    navigator.clipboard.writeText(msg).then(() => { alert(`คัดลอกรายการแล้ว! กำลังพาท่านไปที่ ${platformName} นะคะ ✨`); window.location.href = targetUrl; 
                    }).catch(() => { window.location.href = targetUrl; });
                } else { alert(`กำลังพาท่านไปที่ ${platformName} (อย่าลืมแคปหน้าจอรายการสินค้านะคะ) ✨`); window.location.href = targetUrl; }
        ...
        updateUI() {
            ...
            // ระบบตรวจสอบเปิด-ปิด ป้ายเตือนสั่งของเกิน 10 ชิ้น
            const warningEl = document.getElementById('cart-10-warning');
            if (warningEl) {
                if (selectedItems.length >= 10) warningEl.classList.remove('hidden');
                else warningEl.classList.add('hidden');
            }

            // ถ้าไม่มีของ ให้แสดงว่าตะกร้าว่าง ถ้ามีของให้วาดรายการสินค้าลงไป
            if (!elItems) return;
